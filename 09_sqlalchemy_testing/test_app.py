...
from db import dal
from test_utils import prep_db
...
class TestApp(unittest.TestCase):
    ...
    def test_orders_by_non_existing_cust(self):
        results = get_orders_by_customer('none')
        self.assertEqual(results, [])

    def test_orders_by_non_existing_cust_shipped_only(self):
        results = get_orders_by_customer('none', True)
        self.assertEqual(results, [])

    def test_orders_by_non_existing_cust_unshipped_only(self):
        results = get_orders_by_customer('none', False)
        self.assertEqual(results, [])

    def test_orders_by_non_existing_cust_with_details(self):
        results = get_orders_by_customer('none', details=True)
        self.assertEqual(results, [])

    def test_orders_by_non_existing_cust_shipped_only_with_details(self):
        results = get_orders_by_customer('none', True, True)
        self.assertEqual(results, [])

    def test_orders_by_non_existing_cust_unshipped_only_details(self):
        results = get_orders_by_customer('none', False, True)
        self.assertEqual(results, [])